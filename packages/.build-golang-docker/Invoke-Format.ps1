...
# TASK: format, fmt
# DESCRIPTION: Formats Go source files using gofmt (Docker)
# DEPENDS:

Write-Host "Formatting Go files (Docker)..." -ForegroundColor Cyan
...
if ($env:BOLT_GOLANG_DOCKER_REBUILD -eq "1" -or $env:BOLT_GOLANG_DOCKER_REBUILD -eq "true") {
    Write-Host "  Building Docker image with --no-cache (BOLT_GOLANG_DOCKER_REBUILD=1)..." -ForegroundColor Gray
...
$output = & docker run --rm -v "${absolutePath}:/project" -w /project $dockerImage go fmt ./... 2>&1
