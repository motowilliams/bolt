...
# TASK: test
# DESCRIPTION: Runs Go tests (Docker)
# DEPENDS: format, lint

Write-Host "Running Go tests (Docker)..." -ForegroundColor Cyan
...
$output = & docker run --rm -v "${absolutePath}:/project" -w /project $dockerImage go test -v ./... 2>&1
