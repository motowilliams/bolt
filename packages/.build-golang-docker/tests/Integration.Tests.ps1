...
<#
.SYNOPSIS
    Integration tests for Golang Docker tasks
.DESCRIPTION
    ...
    Tagged with Package-Golang-Tasks-Docker to differentiate from non-Docker variant.
#>
...
Describe 'Docker Task Integration Tests' -Tag 'Package-Golang-Tasks-Docker' {
    Context 'Format Task Integration' {
        It 'Should format Go files in Docker container' {
    ...
    Context 'Lint Task Integration' {
        It 'Should lint Go files in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Test Task Integration' {
        It 'Should run Go tests in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Build Task Integration' {
        It 'Should build Go application in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
