...
<#
.SYNOPSIS
    Pester tests for Golang Docker tasks
.DESCRIPTION
...
Describe 'Task Validation' -Tag 'Package-Golang-Tasks-Docker' {
    Context 'Format Task' {
        It 'Should exist' {
    ...
    Context 'Dockerfile' {
        ...
        It 'Should use Golang base image' {
            $content = Get-Content $script:DockerfilePath -Raw -ErrorAction Stop
            $content | Should -Match 'FROM.*golang'
        }
