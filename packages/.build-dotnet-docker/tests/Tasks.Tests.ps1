...
Describe ".NET Package Starter - Task Validation" -Tag "DotNet-Tasks" {
    ...
    Context "Task Structure" {
        It "format task should have tool detection logic" {
            $content = Get-Content -Path "$packagePath/Invoke-Format.ps1" -Raw
            $content | Should -Match "Get-Command"
        }

        It "restore task should have tool detection logic" {
            $content = Get-Content -Path "$packagePath/Invoke-Restore.ps1" -Raw
            $content | Should -Match "Get-Command"
        }

        It "test task should have tool detection logic" {
            $content = Get-Content -Path "$packagePath/Invoke-Test.ps1" -Raw
            $content | Should -Match "Get-Command"
        }

        It "build task should have tool detection logic" {
            $content = Get-Content -Path "$packagePath/Invoke-Build.ps1" -Raw
            $content | Should -Match "Get-Command"
        }

        It "all task scripts should exit 0 on success" {
            $taskFiles | ForEach-Object {
                $content = Get-Content -Path $_.FullName -Raw
                $content | Should -Match "exit 0" -Because "$($_.Name) should exit 0 on success"
            }
        }

        It "all task scripts should exit 1 on failure" {
            $taskFiles | ForEach-Object {
                $content = Get-Content -Path $_.FullName -Raw
                $content | Should -Match "exit 1" -Because "$($_.Name) should exit 1 on failure"
            }
        }
    }
    ...
    Context "Documentation" {
        ...
        It "README.md should document requirements" {
            $content = Get-Content -Path "$packagePath/README.md" -Raw
