...
<#
.SYNOPSIS
    Integration tests for TypeScript Docker tasks
.DESCRIPTION
    ...
    Tagged with Package-Typescript-Tasks-Docker to differentiate from non-Docker variant.
#>
...
BeforeAll {
    ...
    $script:TypeScriptAppPath = Join-Path $PSScriptRoot 'app'
...
Describe 'Docker Task Integration Tests' -Tag 'Package-Typescript-Tasks-Docker' {
    Context 'Format Task Integration' {
        It 'Should format TypeScript files in Docker container' {
            ...
            Test-Path $script:TypeScriptAppPath | Should -Be $true
    ...
    Context 'Lint Task Integration' {
        It 'Should lint TypeScript files in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Test Task Integration' {
        It 'Should run TypeScript tests in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Build Task Integration' {
        It 'Should build TypeScript application in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
            ...
            $distPath = Join-Path $script:TypeScriptAppPath 'dist'
            Test-Path $distPath | Should -Be $true
        }
    }
