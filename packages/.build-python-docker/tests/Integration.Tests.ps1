...
<#
.SYNOPSIS
    Integration tests for Python Docker tasks
.DESCRIPTION
...
BeforeAll {
    ...
    $script:PythonAppPath = Join-Path $PSScriptRoot 'app'
...
Describe 'Docker Task Integration Tests' -Tag 'Package-Python-Tasks-Docker' {
    Context 'Format Task Integration' {
        It 'Should format Python files in Docker container' {
            ...
            Test-Path $script:PythonAppPath | Should -Be $true
    ...
    Context 'Lint Task Integration' {
        It 'Should lint Python files in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Test Task Integration' {
        It 'Should run Python tests in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
    ...
    Context 'Build Task Integration' {
        It 'Should build Python application in Docker container' {
            if (-not $script:hasDocker) {
                Set-ItResult -Skipped -Because "Docker CLI is not installed"
                return
            }
